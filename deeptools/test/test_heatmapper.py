...
def test_plotProfiler():
    args = "-m {}/master.mat.gz --outFileName /tmp/_test.png --regionsLabel uno dos " \
           "--plotType std".format(ROOT).split()
    deeptools.plotProfile.main(args)
    res = compare_images(ROOT + '/profile_master.png', '/tmp/_test.png', tolerance)
    assert res is None, res
    os.remove('/tmp/_test.png')


def test_plotProfiler_heatmap():
    args = "-m {}/master.mat.gz --outFileName /tmp/_test.png --plotType heatmap".format(ROOT).split()
    deeptools.plotProfile.main(args)
    res = compare_images(ROOT + '/profile_master_heatmap.png', '/tmp/_test.png', tolerance)
    assert res is None, res
    os.remove('/tmp/_test.png')
...
def test_plotProfiler_multibigwig():
    args = "-m {}/master_multi.mat.gz --outFileName /tmp/_test.png " \
           "--numPlotsPerRow 2 --yMax 1.5".format(ROOT).split()
    deeptools.plotProfile.main(args)
    res = compare_images(ROOT + '/profile_master_multi.png', '/tmp/_test.png', tolerance)
    assert res is None, res
    os.remove('/tmp/_test.png')
...
def test_plotProfiler_multibigwig_pergroup():
    ...
    deeptools.plotProfile.main(args)
    res = compare_images(ROOT + '/profile_master_multi_pergroup.png', '/tmp/_test.png', tolerance)
    assert res is None, res
    os.remove('/tmp/_test.png')
