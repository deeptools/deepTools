...
pub fn r_bamcoverage(
    ...
    let offset = match offsetv.as_slice() {
        [a, b] => (*a, *b),
        _ => panic!(
            "Error: Offset should be a list of 2. Received: {:?}",
            offsetv
        ),
    };
    ...
    if mnase {
        if minfraglen == 0 {
            ...
            println!(
                "Note that MNase mode is set, but minfraglen is set at {}. Recommended is 130.",
                minfraglen
            );
        }
        if maxfraglen == 0 {
            ...
            println!(
                "Note that MNase mode is set, but maxfraglen is set at {}. Recommended is 200.",
                maxfraglen
            );
        }
        if binsize != 1 {
            println!(
                "Note that MNase mode is set, but binsize is set at {}. Recommended is 1.",
                binsize
            );
        }
    }
    ...
    if filters.extendreads && filters.extendreadslen == 0 && ispe {
        ...
        if verbose {
            println!(
                "fragment length for read extension set as: {}",
                filters.extendreadslen
            );
        }
    }
    ...
    let (bg, mapped, _unmapped, readlen, fraglen) = pool.install(|| {
        regionblocks
            ...
            .map(|i| {
                bam_pileup(
                    bamifile, &i, &binsize, &ispe, &ignorechr, &filters, collapse, false, true,
                )
            })
            .reduce(
                || (vec![], 0, 0, vec![], vec![]),
    ...
    if filters.extendreads {
        if verbose {
            println!(
                "extend reads option on, overriding readlen from {} to {}",
                readlen, filters.extendreadslen
            );
        }
