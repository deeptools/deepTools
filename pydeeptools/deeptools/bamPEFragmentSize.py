...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "-hist",
        "-o",
        help="Save a .png file with a histogram of the fragment length distribution.",
        metavar="FILE",
    )
    ...
    parser.add_argument(
        "--ggplot", help="Use ggplot theme for figures", action="store_true"
    )
    ...
    parser.add_argument("--logScale", help="Plot on the log scale", action="store_true")
    parser.add_argument(
        "--binSize",
        "-bs",
        metavar="INT",
...
def getFragSize(bam, args, idx, outRawFrags):
    ...
    if args.histogram:
        ...
        if read_len_dict:
            read_len_dict["lengths"] = getDensity(
                read_len_dict["lengths"], minVal, maxVal
            )
    else:
        if fragment_len_dict:
