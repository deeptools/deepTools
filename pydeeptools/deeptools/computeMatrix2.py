...
def computeMatrixOptArgs(case=["scale-regions", "reference-point"][0]):
    ...
    if case == "scale-regions":
        ...
        optional.add_argument(
            "--regionBodyLength", help=argparse.SUPPRESS, default=0, type=int
        )
        optional.add_argument(
            "--unscaled5prime", default=0, type=int, help=argparse.SUPPRESS
        )
        optional.add_argument(
            "--unscaled3prime", default=0, type=int, help=argparse.SUPPRESS
        )
        optional.add_argument(
            "--beforeRegionStartLength",
            "-b",
    ...
    optional.add_argument(
        "--verbose",
        help="Being VERY verbose in the status messages.",
        action="store_true",
    )

    optional.add_argument(
        "--scale",
        help="If set, all values are multiplied by this number. (Default: %(default)s)",
        type=float,
        default=1,
    )
    optional.add_argument(
...
def process_args(args=None):
    ...
    if args.beforeRegionStartLength < 0:
        print(
            f"beforeRegionStartLength changed from {args.beforeRegionStartLength} into {abs(args.beforeRegionStartLength)}"
        )
        args.beforeRegionStartLength = abs(args.beforeRegionStartLength)
    if args.afterRegionStartLength < 0:
        print(
            f"afterRegionStartLength changed from {args.afterRegionStartLength} into {abs(args.afterRegionStartLength)}"
        )
